...
class _GamepadDeckState extends State<GamepadDeck> with WidgetsBindingObserver {
      ...
      child: Stack(
        children: [
          ...
          Positioned(
            left: size,
            top: 0,
            child: _buildDPadDirection(label: 'Γû▓', buttonId: 1, width: size, height: size),
          ),
          // DOWN
          Positioned(
            left: size,
            top: size * 2,
            child: _buildDPadDirection(label: 'Γû╝', buttonId: 2, width: size, height: size),
          ),
          // LEFT
          Positioned(
            left: 0,
            top: size,
            child: _buildDPadDirection(label: 'ΓùÇ', buttonId: 3, width: size, height: size),
          ),
          // RIGHT
          Positioned(
            left: size * 2,
            top: size,
            child: _buildDPadDirection(label: 'Γû╢', buttonId: 4, width: size, height: size),
          ),
  ...
  Widget _buildPs1Cluster(double size) {
    ...
    return SizedBox(
      ...
      child: Stack(
        children: [
          Positioned(left: 0, top: spacing / 2, child: _buildGamepadButton(label: 'Γûí', buttonId: 8, color: const Color(0xFFE91E63), size: size)), // Square = Y
          Positioned(left: spacing / 2, top: 0, child: _buildGamepadButton(label: 'Γû│', buttonId: 7, color: const Color(0xFF4CAF50), size: size)), // Triangle = X
          Positioned(left: spacing, top: spacing / 2, child: _buildGamepadButton(label: 'Γùï', buttonId: 5, color: const Color(0xFFF44336), size: size)), // Circle = A
          Positioned(left: spacing / 2, top: spacing, child: _buildGamepadButton(label: 'Γ£ò', buttonId: 6, color: const Color(0xFF2196F3), size: size)), // Cross = B
        ],
      ),
    );
  }
